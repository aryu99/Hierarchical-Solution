...
class Glue:
    def __init__(self, verbose=False):
        ...
        self.obs, self.req_queue = self.get_sim_state(verbose)
        self.abstract_sim = AbstractSimulator(self.obs, goal_coords, shelf_coords, self.req_queue)

    def get_env(self):
        env = make_env()
        return env

    def get_sim_state(self, verbose=False):
        '''
        ...
        obs = self.env.reset()
        req_queue = self.env.request_queue
    ...
    def get_current_state(self, verbose=False):
        ...
        return self.abstract_sim.get_current_state()
...
# Simulator API
    def GetNextState(self, CurrState):
        ...
        self.abstract_sim.reset_state(CurrState)
        Actions = self.abstract_sim.get_actions()
        Action = {}
        ...
        # Get the next state
        NextState = self.abstract_sim.execute_action(Action)  

        return NextState

    def EvalNextStates(self, CurrState):
        '''
        ...
        self.abstract_sim.reset_state(State)
        Actions = self.abstract_sim.get_actions()
        ...
        for m in range(len(storeActions)):
            ...
            if all(x == list(storeActions[m].values())[0] for x in storeActions[m].values()):
                if list(storeActions[m].values())[0][0].value == 2:
                    self.abstract_sim.reset_state(copy.deepcopy(State))      
                    NextStates.append((self.abstract_sim.execute_action(storeActions[m]), storeActions[m])) 
